...
let
  ...
  mkVirtualDisplay = idx: displayCfg: let
    ...
    edidName = "sunshine-virtual-${toString idx}.bin";
  in {
  ...
  virtualDisplays = imap0 mkVirtualDisplay cfg.displays;

  # Type pour un écran virtuel
  displayType = types.submodule {
    options = {
      videoOutput = mkOption {
        type = types.str;
        description = ''
          The video output to use for this virtual display.
          Find available outputs with: for p in /sys/class/drm/*/status; do con=''${p%/status}; echo "''${con#*/card?-}: $(cat $p)"; done
        '';
        example = "HDMI-A-1";
      };
...
in
{
  options.services.sunshine-virtual-display = {
    enable = mkEnableOption "virtual display(s) for Sunshine streaming";
  ...
  config = mkIf cfg.enable {
    ...
    boot.initrd.extraFiles = listToAttrs (map (vd:
      nameValuePair "lib/firmware/edid/${vd.edidName}" vd.edidFile
    ) virtualDisplays);
    ...
    environment.systemPackages =
      let
          ...
          '' else ''
            ...
            in ''
              ...
              ${pkgs.xorg.xrandr}/bin/xrandr \
                ...
                ${optionalString isPrimary "--primary"} 2>/dev/null || \
                echo "⚠️  Échec de l'activation de ${vd.output}"
            '') virtualDisplays}
          ''}
        ...
        disableVirtualScript = pkgs.writeShellScriptBin "sunshine-stop" ''
          ...
          '' else ''
            ...
            ${pkgs.xorg.xrandr}/bin/xrandr --output ${head cfg.physicalOutputs} --primary 2>/dev/null || true
          ''}
      ...
      in [
        ...
        infoScript
        listOutputsScript
      ] ++ optional cfg.useWayland pkgs.wlr-randr;
  };
}
