...
| `mx.programs.*`                                               | `audio-enhancer`, `cryptomator`, `firefox`, `git`, `graphism.{gimp,inkscape,krita}`, `kdrive`, `linux-base-tools`, `office.{latex,libre-office,onlyoffice}`, `ssh`, `thunderbird`, `video-downloader`, `vim`, `vscode`, `winboat`, `zed-editor`, `dev.*` |
